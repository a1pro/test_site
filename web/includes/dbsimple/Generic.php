 ...
 *
 * Modified by Alex Scott <alex@cgi-central.net> , 2007
 * - added _queryResultOnly() to get only query pointer 
 * - changed selectCol (see code) to return back version that is 
 *   working in PHP4
 *
...
class DbSimple_Generic_Database extends DbSimple_Generic_LastError
{
    ...
    function selectCol()
    {
        ...
        $rows = $this->_query($args, $total);
        if (!is_array($rows)) return $rows;
/*      //Reverted back this code to code below 
  		// that does not support multi-dimension, but works in PHP4
...
*/
     ...
     * See _performQuery().
     */
    function _query($query, &$total, $resultOnly=false)
    {
        $this->_resetLastError();
        ...
        if (null === $rows || true === $cache_it) {
            ...
            $result = $this->_performQuery($query, $resultOnly);
            $fetchTime = $firstFetchTime = 0;
            
            if (is_resource($result) && !$resultOnly) { // added by Alex (temp. dirty hack)
                $rows = array();
