 ...
 *
 * Modified by alex@cgi-central.net - added _queryResultOnly and aMember-specific code
 * It is not subclassed for optimization and because little internal changes anyway required.
 *
...
{
    var $link;
    var $_query_result = array(); //stack. added by Alex for aMember's internal needs

    // added by alex@cgi-central.net
    function queryResultOnly($query){
        $args = func_get_args();
        $total = false;
        $ret = $this->_query($args, $total=null, $resultOnly = true);
		$this->_return_query_total = $total;
    	return array_pop($this->_query_result);
    }
    // added by alex@cgi-central.net
    function fetchRow($q){
    ...
    // added by alex@cgi-central.net
    /**
     * This function implements caching of already discovered records 
     * @return TableStruct object with table structure
     */
    var $_tables = array();
    function resetTableStructList(){
    	$this->_tables = array();
    }
    function & getTableStruct($table){
        if (!array_key_exists($table, $this->_tables)){
            $this->_tables[$table] = $this->_createTableStruct($table);
        }
        return $this->_tables[$table];
    }
    // added by alex@cgi-central.net
    function _createTableStruct($table){
        $s = & new SDP_MySQLTableStruct($this, SDP_Core::getTablesPrefix() . $table);
        return $s;
    }
    function quote($s){
    ...
    function _performQuery($queryMain, $resultOnly = false)
    {
        $this->_lastQuery = $queryMain;
        $this->_expandPlaceholders($queryMain, false);
        $result = @mysql_query($queryMain[0], $this->link);
        if ($resultOnly) $this->_query_result[] = $result;
