...
*    FileName $RCSfile$
*    Release: 3.2.3PRO ($Revision: 5314 $)
*
...
*	Revision History:
*	----------------
*	2010-10-23	v3.2.3.0	K.Gary	Merged with aMember v3.2.3 for aMail plugin.
*	2010-09-14	v3.2.2.0	K.Gary	Merged with aMember v3.2.2
*	2010-08-25	v3.1.9.1	K.Gary	Merged with security release.
*	2010-04-19	v3.1.9.0	K.Gary	amSalesPages applied changes to 3.1.9
*	2009-09-09	v3.1.8.0	K.Gary	amSalesPages original product creation.
*
* =============================================================================
*
*/
...
class db_mysql extends amember_db {
    ...
    function add_pending_user($vars){
	    ...
	    //
	    // Added for amSalesPages Plugin...
	    //
		if (function_exists(check_setup_amsalespages))
		{
        	// BEGIN MOD TO GET UNSUBSCRIBED RIGHT FROM SIGNUP FORM
        	$vars['unsubscribed'] = (empty($vars['to_subscribe'])) ? 1 : 0;
        	// END MOD TO GET UNSUBSCRIBED RIGHT FROM SIGNUP FORM
		}
		//
	    // End amSalesPages Plugin
	    //
		...
		// Begin amOneClick Mod
		if (!empty($vars['country'])) $data['cc_country'] = $vars['country'];
		if (!empty($vars['state'])  ) $data['cc_state']   = $vars['state'];
		if (!empty($vars['street']) ) $data['cc_street']  = $vars['street'];
		if (!empty($vars['city'])   ) $data['cc_city']    = $vars['city'];
		if (!empty($vars['zip'])    ) $data['cc_zip']     = $vars['zip'];
		if (!empty($vars['name_f']) ) $data['cc_name']    = $vars['name_f'].' '.$vars['name_l'];
		if (!empty($vars['phone'])  ) $data['cc_phone']   = $vars['phone'];
		// End amOneClick Mod
    ...
    function get_users_list_c($pattern='%', $status=-1,$aff_id=0){
        $where = $having = "";
        if ($pattern)
            $where = " AND u.login LIKE '" . $this->escape($pattern) . "' ";
        if (strlen($status)){
    ...
    function users_find_by_string($q, $q_where, $exact=0){
        ...
        foreach ($member_additional_fields as $f)
        	if ($f['sql']){
        		$sql_fields[$f['name']]++;
        		$additional_where.=" OR (u.$f[name] LIKE '%$q%')";
        	}
        ...
        switch ($q_where){
            ...
            default: $where_exp = " AND
                ...
                (u.zip LIKE '%$q%') OR
                (u.country LIKE '%$q%') OR
                (u.remote_addr LIKE '%$q%') $additional_where
                 ";
        }
        $q = $this->query($s = "SELECT u.*,
            SUM(if(p.completed, 1, 0)) AS count_of_completed,
...
//        settype($product_id, 'integer');
    ...
    function add_waiting_payment($member_id, $product_id, $paysys_id,
        ...
        if ($product_id<=0)
            fatal_error("Empty or incorrect product# passed");
    ...
    function add_waiting_payments($member_id, $product_id, $paysys_id,
            $price, $_prices, $begin_date, $expire_date, $vars,
            $additional_values=false){
        $prices = array();
        foreach ((array)$_prices as $k => $v) $prices[intval($k)] = $v;
    ...
    function update_payment($payment_id, &$v){
        ...
        if (($v['completed']>0) && !$old['completed'])
            plugin_finish_waiting_payment($payment_id, $member_id);
        	if ($coupon = $oldp['data'][0]['COUPON_CODE']){
                $this->coupon_used($coupon, $payment_id, $oldp['product_id'],
                    $oldp['member_id']);
            }
    ...
    function get_payments_c($beg_date, $end_date, $only_completed, $list_by='',
        ...
        $q = $this->query($s = "SELECT COUNT(p.payment_id), SUM(p.amount),SUM(p.tax_amount)
            FROM {$this->config['prefix']}payments p
                LEFT JOIN {$this->config['prefix']}members m USING (member_id)
                LEFT JOIN {$this->config['prefix']}products pr ON (p.product_id = pr.product_id)
            WHERE $search_add $where_add $src_pr
            ");
        list($c, $s, $ta) = mysql_fetch_row($q);
        return array($c, $s,$ta);
    }
...
//            return;
    ...
    function get_coupons($where, $what, $start=0, $limit=-1){
        $where_add = '';
    ...
    function get_coupons_c($where, $what){
        $where_add = '';
        $what = $this->escape($what);
        switch ($where){
            case 'coupon_id': $where_add = "AND coupon_id = '$what'";
        ...
        $count = $this->query_one($s = "SELECT count(*)
            FROM {$this->config['prefix']}coupon
            WHERE 1 $where_add
            ");
        return $count;
            ...
            if ($r['threads']) {
                ...
                while ( list (, $thread_id) = each ($threads) ) {
                    if ($thread_id > 0){
                        ...
                        if ($member_id > 0){
                        	if($this->is_thread_available_to_member($thread_id, $member_id)){
                ...
                if(!$real_threads)$is_available = true;
...
//                "<br />in query:<br />$s</font>";
                ...
                $this->log_error("MYSQL ERROR:<br />" . mysql_error($this->conn) .
