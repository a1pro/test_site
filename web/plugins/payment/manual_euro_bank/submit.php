...
function ILVX_WS_ASCII_MODE_VERIFY ($vBLZ, $vKonto, $vMode, $vAccess, $vSessionID, $vTimeStamp)
{
      $a_vResult[VerifyProject] = "ilvx";
      $a_vResult[VerifyMode] = $vMode;
      $a_vResult[VerifySID] = $vSessionID;
      $a_vResult[VerifyAccount] = $vKonto;
      $a_vResult[VerifyBLZ] = $vBLZ;
      $a_vResult[VerifyResult] = "129";
...
function MarkC2($AccountNo) {
  ...
  function CheckAccount($banktransfer_number, $banktransfer_blz) {
    ...
    $vAccess = "0000000000000000000000000000000000000000";

    $vTimeStamp = date("Y-m-d\TH:i:s") . "," . substr (microtime(), 2, 3);

    $SidBuilder = $vAccess . $BLZ . $KontoNR . $vTimeStamp;
    $vSessionID = strtoupper(substr(md5($SidBuilder), 0, 30));

    $a_vIlvxWsResult = $this->ILVX_WS_ASCII_MODE_VERIFY ($BLZ, $KontoNR, "0", $vAccess, $vSessionID, $vTimeStamp);

    $this->VerifyWarning = $a_vIlvxWsResult[VerifyResult];
