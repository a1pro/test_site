...
function do_main(){
    ...
    while ($vars['do_cc']){
        ...
        if (!is_array($m)) 
            fatal_error(sprintf(_PLUG_PAY_NETBILLING_FERROR4, $member_id, $payment_id));
...
//        exit();
        ...
        if ($vars['renew_cc']) {
            ...
            header("Location: $config[root_surl]/plugins/payment/netbilling/cc.php?".
...
function validate_cc_info(&$vars){
    ...
    $cc = $vars['cc_number'];
    if (strlen($cc) < 12) {
        $errors[] = _PLUG_PAY_NETBILLING_ERROR;
    }
    if (($vars['cc_expire_Month'] < 1) or ($vars['cc_expire_Month']>12)){
        $errors[] = _PLUG_PAY_NETBILLING_ERROR2;
    }

    if ($vars['cc_expire_Year'] < date('Y')){
        $errors[] = _PLUG_PAY_NETBILLING_ERROR3;
    }

    if (!strlen($vars['cc_code']))
        $errors[] = _PLUG_PAY_NETBILLING_ERROR4;

    if (!strlen($vars['cc_street']))
        $errors[] = _PLUG_PAY_NETBILLING_ERROR5;
    if (!strlen($vars['cc_city']))
        $errors[] = _PLUG_PAY_NETBILLING_ERROR6;
    if (!strlen($vars['cc_state']))
        $errors[] = _PLUG_PAY_NETBILLING_ERROR7;
    if (!strlen($vars['cc_zip']))
        $errors[] = _PLUG_PAY_NETBILLING_ERROR8;
    if (!strlen($vars['cc_country']))
        $errors[] = _PLUG_PAY_NETBILLING_ERROR9;
    return $errors;
}
