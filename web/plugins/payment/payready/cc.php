...
function do_main(){
    ...
    $cc_fields = array('street', 'city', 'state', 'zip','country', 'type');
    ...
    if ($vars['renew_cc']){ // cc info renewal, require auth, don't req. payment_id
        if ($member_id != $_SESSION['_amember_id']){
            fatal_error(_PLUG_PAY_PAYRDY_FERROR, 0);
        }
    } else { //regular payment
        $payment_id = intval($vars['payment_id']);
        if (!$payment_id)
            fatal_error(_PLUG_PAY_PAYRDY_FERROR2);
    }
    ...
    while ($vars['do_cc']){
        ...
        if (!is_array($m)) 
            fatal_error(sprintf(_PLUG_PAY_PAYRDY_FERROR3, $member_id, $payment_id));
...
//        exit();
        ...
        if ($vars['renew_cc']) {
            ...
            header("Location: $config[root_surl]/plugins/payment/payready/cc.php?".
...
#        fatal_error("Your payment is being processed - To avoid double charges, please DO NOT PRESS THE SUBMIT BUTTON AGAIN");
    ...
    if ($errno == 2 || $errno == 3){
        global $t;
        $t->assign('error', $err);
        $t->assign('pay_link', "$config[root_surl]/plugins/payment/payready/cc.php?payment_id=$payment_id&member_id=$member_id&retry=1");
...
function validate_cc_info(&$vars){
    ...
    if (!strlen($vars['cc_type'])) {
        $errors[] = _PLUG_PAY_PAYRDY_ERROR;
    }
    $cc = $vars['cc_number'];
    if (strlen($cc) < 12) {
        $errors[] = _PLUG_PAY_PAYRDY_ERROR2;
    }
    if (($vars['cc_expire_Month'] < 1) or ($vars['cc_expire_Month']>12)){
        $errors[] = _PLUG_PAY_PAYRDY_ERROR3;
    }

    if ($vars['cc_expire_Year'] < date('Y')){
        $errors[] = _PLUG_PAY_PAYRDY_ERROR4;
    }
...
//        $errors[] = "Please enter Credit Card Code";
    ...
    if (!strlen($vars['cc_street']))
        $errors[] = _PLUG_PAY_PAYRDY_ERROR5;
    if (!strlen($vars['cc_city']))
        $errors[] = _PLUG_PAY_PAYRDY_ERROR6;
    if (!strlen($vars['cc_state']))
        $errors[] = _PLUG_PAY_PAYRDY_ERROR7;
    if (!strlen($vars['cc_zip']))
        $errors[] = _PLUG_PAY_PAYRDY_ERROR8;
    if (!strlen($vars['cc_country']))
        $errors[] = _PLUG_PAY_PAYRDY_ERROR9;
    return $errors;
}
