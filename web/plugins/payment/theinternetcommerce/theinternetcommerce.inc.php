...
*    Details: TIC payment plugin class
*    FileName $RCSfile$
...
add_product_field('theinternetcommerce_currency', 
    'TheInternetCommerce Currency',
    'select',
    'valid only for TheInternetCommerce processing.<br /> You should not change it<br /> if you use 
...
array(
            'paysys_id' => 'theinternetcommerce',
            'title'     => $config['payment']['theinternetcommerce']['title'] ? $config['payment']['theinternetcommerce']['title'] : _PLUG_PAY_INETCOM_TITLE,
            'description' => $config['payment']['theinternetcommerce']['description'] ? $config['payment']['theinternetcommerce']['description'] : _PLUG_PAY_INETCOM_DESC,
            'public'    => 1,
            'recurring' => 1
        )
);
...
function theinternetcommerce_get_member_links($user){
    global $config;
    if ($user['data']['cc'])
        return array("$config[root_surl]/plugins/payment/theinternetcommerce/cc.php?renew_cc=1&member_id=$user[member_id]" => 'Update CC info');
}

// return 1 if rebill allowed
function theinternetcommerce_check_rebill_times($times, $payment){
    global $db;
...
function theinternetcommerce_rebill(){
    global $config, $db, $t;
    if (!$config['use_cron'])
        fatal_error("TheInternetCommerce rebill can be run only with external cron");
    $dat = date('Y-m-d');
    $tomorrow = date('Y-m-d', time() + 3600 * 24);
    $payments = $db->get_expired_payments($dat, $dat, 'theinternetcommerce');
    $renewed = array();
    $log = "TheInternetCommerce Rebill\n";
    foreach ($payments as $p){
        ...
        if ($product->config['rebill_times'] &&
            !theinternetcommerce_check_rebill_times($product->config['rebill_times'], $p))
            continue;
        $vars = array(
            'RENEWAL_ORIG' => "RENEWAL_ORIG: $p[payment_id]"
        );
        $payment_id = $db->add_waiting_payment($member_id, $product_id, 
            'theinternetcommerce', $product->config['price'], 
        ...
        if ($errno>1){
            ...
            $db->log_error("no return from TheInternetCommerce, payment #$payment_id");
        } elseif ($errno == 1) {
            $err = "COMPLETED";
