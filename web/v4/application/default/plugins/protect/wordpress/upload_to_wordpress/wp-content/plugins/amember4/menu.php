...
class am4Menu extends am4Plugin{
    ...
    function action_AdminMenu(){
        add_menu_page('aMember', 'aMember', 'administrator', $this->slug);
    ...
    function action_AdminEnqueueScripts(){
    }
...
class am4MenuPage_settings extends am4MenuPageFormController{
    ...
    function getOptions(){
        $options = am4Settings::getInstance()->getAll();
        if(empty($options['user_action']) || empty($options['guest_action'])) $options = array_merge($options, $this->getProtectionDefaults());
        return $options;
    }
...
class am4MenuPage_styles extends am4MenuPageFormController{
    ...
    function saveForm($options){
        ...
        if($options) update_option(AM4_STYLE_OPTION, $options);
    }
    
    function getOptions(){
        $options = get_option(AM4_STYLE_OPTION);
        foreach($this->styles as $s){
            if(!$options[$s]){
                $options[$s] = file_get_contents(AM4_PLUGIN_DIR."/views/".$s);
            }
        }
...
class am4MenuPage_errormessages extends am4MenuPageFormController{
    ...
    function saveForm($options){
        ...
        if($options['id']){
            $errors[$options['id']] = $options;
        }else{
            $errors[] = $options;
        }
        update_option(AM4_ERROR_OPTION, $errors);
    }
    
    function getOptions(){
        $options = get_option(AM4_ERROR_OPTION);
        return ($options ? $options : array());
    }
