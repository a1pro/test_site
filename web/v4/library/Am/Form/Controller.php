...
class Am_Form_Controller extends HTML_QuickForm2_Controller
{
    ...
    public function setParentController(Am_Controller $parent)
    {
        $this->parent = $parent;
    }
    /** @return Am_Controller */
...
class Am_Form_Controller_SessionContainer extends HTML_QuickForm2_Controller_SessionContainer
{
    ...
    public function __construct(HTML_QuickForm2_Controller $controller) {
        ...
        $this->name = sprintf(HTML_QuickForm2_Controller::KEY_CONTAINER,
                        $controller->getId());
        /* @var $session Zend_Session_Namespace */
        if (empty($_SESSION['amember'][$this->name]))
        {
            $_SESSION['amember'][$this->name] = array(
                'datasources' => array(),
                'values'      => array(),
                'valid'       => array()
            );
    ...
    {
        $_SESSION['amember'][$this->name] = unserialize($data);
        $this->data =& $_SESSION['amember'][$this->name];
    }
    function destroy()
    {
        unset($_SESSION['amember'][$this->name]);
    }
}
...
class Am_Form_Controller_Action_Jump implements HTML_QuickForm2_Controller_Action
{
    ...
    */
    ...
    public function perform(HTML_QuickForm2_Controller_Page $page, $name)
    {
        ...
        $page->getController()->getParentController()->redirectLocation($action . (false === strpos($action, '?')? '?': '&') .
...
/** call Am_Controller's function process() */
