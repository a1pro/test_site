...
class Am_Grid_Editable extends Am_Grid_ReadOnly
{
    ...
    public function getFields()
    {
        ...
        if (!$fieldActionsAdded++)
        {
            if ($this->getActions(Am_Grid_Action_Abstract::SINGLE))
                $this->addField(new Am_Grid_Field_Actions);
            if ($this->getActions(Am_Grid_Action_Abstract::GROUP))
                $this->prependField(new Am_Grid_Field_Checkboxes);
            $fieldActionsAdded = 1;
        }
        return $this->fields;
    }
    ...
    public function initForm()
    {
        ...
        $this->form->addSubmit('save', array('value' => ___("Save")));
    }
    ...
    {
        if ($actionName !== null)
            return sprintf($actionName, $this->recordTitle);
        else
    ...
    public function renderButtons()
    {
        ...
        {
            $out .= sprintf('<input type="button" id="%s" value="%s" onclick="window.location=%s" >' . PHP_EOL,
                $this->getCssClass() . '-' . $action->getId() . '-button',
                $this->escape($this->getRecordTitle($action->getTitle())) ,
                $this->escape(Am_Controller::getJson($action->getUrl()))
                );
        }
        $out .= "</div>" . PHP_EOL;
        return $out;
    ...
    function getRecordId()
    {
        ...
        if (!$id) throw new Am_Exception_InputError("Wrong link - empty id in " . __METHOD__);
        return $id;
    }
