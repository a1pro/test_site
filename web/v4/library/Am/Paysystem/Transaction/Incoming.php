...
abstract class Am_Paysystem_Transaction_Incoming extends Am_Paysystem_Transaction_Abstract
{
    ...
    {
        if (!$products = $this->autoCreateGetProducts())
            return null;
        if (!is_array($products))
            $products = array($products);
        
        // check if we can find matching products, else exists
        ...
        if ($invoice = Am_Di::getInstance()->invoiceTable->findFirstByData('external_id', $invoiceExternalId))
        {
