...
class Am_Period {
    ...
    function getText($format="%s", $skip_one_c = false){
        ...
        switch ($this->unit){
            case 'd': $uu = $this->count==1 ? 'day': 'days'; break;
            case 'm': $uu = $this->count==1 ? 'month' : 'months'; break;
            case 'y': $uu = $this->count==1 ? 'year' : 'years'; break;
            case self::FIXED: return " up to " . amDate($this->count);
        }
        $cc = $this->count;
        if ($this->count == 1) $cc = $skip_one_c ? '' : 'one';
        return sprintf($format, "$cc $uu");
    }
    function isEmpty() { return empty($this->count) || empty($this->unit); }
    function isRecurring() { return $this->count == self::RECURRING_SQL_DATE; }
    function isLifetime() { return $this->count == self::MAX_SQL_DATE; }
