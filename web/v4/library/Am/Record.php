 ...
 *    Details: Am_Record Class
 *    FileName $RCSfile$
 *    Release: 4.1.10 ($Revision: 3953 $)
 *
...
class Am_Record
{
    ...
    function pk()
    {
        ...
        return isset($this->$keyField) ? filterId($this->$keyField) : null;
    }
    ...
    public function serializeIds($ids)
    {
        ...
        return implode(',', array_filter(array_map('filterId', $ids)));
    }
    ...
    {
        return array_filter(array_map('filterId', explode(',', $ids)));
    }
...
class Am_Table
{
    ...
    {
        $ids = array_filter(array_map('filterId', $ids));
        if (!$ids) return array();
